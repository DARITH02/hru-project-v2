...
<div class="teacher-qr-workspace">
    <header class="teacher-qr-header">
        <div>
            ...
            <p>Display the session 1 QR for the assigned teacher. After scanning, the teacher enters their teacher code to submit attendance.</p>
        </div>
    ...
    <section class="teacher-qr-filter">
        <form method="GET" action="{{ route('admin.teacher-attendance.scan-qr') }}">
            <label>
                <span>Date</span>
            ...
            <label>
                ...
                <select name="session_id">
                    ...
                    @empty
                        <option value="">No session 1 teacher classes for this date</option>
                    @endforelse
                </select>
            </label>
                ...
                <div class="teacher-qr-body">
                    ...
                    <div class="teacher-qr-refresh">
                        ...
                        <div>
                            ...
                            <span>Token auto-regenerates every 60 seconds</span>
                        </div>
    ...
    @else
        <div class="teacher-qr-empty">
            No eligible session 1 teacher classes found for {{ $date->format('M d, Y') }}.
        </div>
    @endif
</div>
@endsection
...
<style>
    ...
    .teacher-qr-filter form{display:grid;grid-template-columns:180px minmax(260px,1fr) auto;gap:12px;align-items:end}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        let payload = @json($teacherScanUrl);
        ...
        const tokenCode = document.getElementById('teacherQrTokenCode');
        const scanUrl = document.getElementById('teacherScanUrl');
        const openScanPage = document.getElementById('openScanPage');
        const refreshButtons = [document.getElementById('refreshQrButton'), document.getElementById('refreshQrInline')].filter(Boolean);
        ...
        async function refreshQr() {
            ...
            try {
                ...
                remaining = Number(data.qr.ttl_seconds) || total;
                ...
                if (label) label.textContent = 'Refreshes in';
                if (seconds) seconds.textContent = `${remaining}s`;
                if (progress) {
